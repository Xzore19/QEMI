...
qr_91fc84 = QuantumRegister(2)
cr_91fc84 = ClassicalRegister(2)
qc.add_register(qr_91fc84)
qc.add_register(cr_91fc84)
qc.x(qr_91fc84[0])
qc.x(qr_91fc84[1])
qc.measure(qr_91fc84[0], cr_91fc84[0]) 
qc.measure(qr_91fc84[1], cr_91fc84[1]) 
with qc.if_test((cr_91fc84, 0b11)) as else_91fc84: 
	pass
with else_91fc84: 
	
	qr_c7d90b = QuantumRegister(2)
	cr_c7d90b = ClassicalRegister(2)
	qc.add_register(qr_c7d90b)
	qc.add_register(cr_c7d90b)
	qc.h(qr_c7d90b[0])
	qc.cx(qr_c7d90b[0], qr_c7d90b[1])         
	qc.measure(qr_c7d90b[0], cr_c7d90b[0]) 
	qc.measure(qr_c7d90b[1], cr_c7d90b[1]) 
	with qc.switch(cr_c7d90b) as case: 
		with case(0b00, 0b11): 
			qc.append(U3Gate(1.614, 5.701, 3.367), [qreg[1]])
			qc.append(C3XGate(), [qreg[1], qreg[3], qreg[2], qreg[4]])
			qc.append(Permutation(3, pattern=[1, 0, 2]), [qreg[1], qreg[3], qreg[0]])
			qc.append(MCXGate(4), [qreg[4], qreg[0], qreg[3], qreg[1], qreg[2]])
		with case(case.DEFAULT): 
			qc.append(RXGate(2.346), [qreg[0]])
			qc.append(U3Gate(0.088, 4.3, 4.77), [qreg[3]])
			qc.append(CCXGate(), [qreg[0], qreg[1], qreg[3]])
			qc.append(TwoLocal(2, reps=1, parameter_prefix='theta_3f8104'), [qreg[4], qreg[2]])
	qc.reset(qr_c7d90b)
