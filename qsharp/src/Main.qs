...
function __RandomFlag_e79f3195() : Bool {
    ...
    operation ApplyRandomBlock0(q : Qubit[]) : Unit {
        // --- DEADCODE START ---
        let target = [q[1], q[2], q[3], q[4], q[5], q[6], q[7], q[8], q[9], q[10], q[11]]; 
        operation __InlineApplyIfRelation_5e65f1cf(q : Qubit[]) : Unit is Adj + Ctl {
            operation __InlineApplyIfRelationL_2da5c9c3(q : Qubit[]) : Unit is Adj + Ctl {
                    operation __GenBlock_9528ae37(q : Qubit[]) : Unit is Adj + Ctl {
            
                MAJ(q[0], q[5], q[4]);
                ApplyCNOTChain([q[6], q[0], q[2]]);
                R1Frac(3, 10, q[6]);
                MAJ(q[3], q[5], q[0]);
                SWAP(q[4], q[6]);
                RippleCarryTTKIncByLE([q[5]], [q[0], q[2], q[4]]);
                IncByLEUsingAddLE(LookAheadDKRSAddLE, RippleCarryCGAddLE, [q[1], q[2], q[4]], [q[0], q[5], q[6]]);
            }
                    Controlled Adjoint __GenBlock_9528ae37([q[1], q[8]], [q[0], q[2], q[3], q[4], q[5], q[6], q[7]]);
            }
            let x = [q[4], q[9]];
            let target = [q[0], q[1], q[2], q[3], q[5], q[6], q[7], q[8], q[10]];
            ApplyIfLessOrEqualL(__InlineApplyIfRelationL_2da5c9c3, 2L, x, target);
        }
        use x = Qubit[2];
        X(x[0]);
        X(x[1]);
        Controlled Adjoint ApplyIfGreaterL([q[0]], (__InlineApplyIfRelation_5e65f1cf, 0L, x, target));
        X(x[0]);
        X(x[1]);
        // --- DEADCODE END ---
    }
