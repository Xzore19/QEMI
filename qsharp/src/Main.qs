    ...
    operation ApplyRandomBlock0(q : Qubit[]) : Unit is Adj + Ctl {
        operation __InlineIfElseDeadcode_d9dafd0c(q : Qubit[]) : Unit is Adj + Ctl {
            if __RandomFlag_3f281223() {
                // --- DEADCODE START ---
                    operation __IfBody_a7d83b42(q : Qubit[]) : Unit is Adj + Ctl {
                            operation __InlineApplyIfRelationL_725de1e1(q : Qubit[]) : Unit is Adj + Ctl {
                                    R1Frac(2, 1, q[0]);
                                    ApplyPauli([PauliX, PauliY], [q[0], q[1]]);
                                    RFrac(PauliX, 10, 2, q[1]);
                                    ApplyQFT(q);
                                    ApplyQFT(q);
                                    ApproximatelyPreparePureStateCP(
                                1e-6,
                                [
                                    ComplexPolar(0.467041, 0.112274),
                                    ComplexPolar(0.603854, 4.891384),
                                    ComplexPolar(0.568456, 3.03642),
                                    ComplexPolar(0.306742, 2.234975)
                                ],
                                q
                            );
                                    ApproximatelyPreparePureStateCP(
                                1e-6,
                                [
                                    ComplexPolar(0.523753, 2.685472),
                                    ComplexPolar(0.22821, 3.787602),
                                    ComplexPolar(0.673801, 3.319958),
                                    ComplexPolar(0.46861, 5.587824)
                                ],
                                q
                            );
                            }
                            let x = [q[2], q[3], q[6]];
                            let target = [q[1], q[5]];
                            ApplyIfLessOrEqualL(__InlineApplyIfRelationL_725de1e1, 5L, x, target);
                    }
                    
                    operation __ElseBody_a7d83b42(q : Qubit[]) : Unit is Adj + Ctl {
                                ApplyCNOTChain([q[5], q[6], q[7], q[3], q[4], q[2]]);
                                R1Frac(2, 1, q[5]);
                                ApplyCNOTChain([q[5], q[7], q[6], q[4], q[1], q[0], q[3], q[2]]);
                                ApplyToEachCA(H, q);
                                ApplyToEachCA(H, q);
                                ApplyToEachCA(MySingleBlock_25bf2541, q);
                                RFrac(PauliX, 12, 2, q[1]);
                    }
                    
                    if __RandomFlag_22227e85() {
                        __IfBody_a7d83b42(q);
                    } else {
                        __ElseBody_a7d83b42(q);
                    }
                // --- DEADCODE END ---
            } else {
