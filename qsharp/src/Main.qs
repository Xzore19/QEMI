namespace Main {
    ...
    operation ApplyRandomBlock0(q : Qubit[]) : Unit {
        // --- DEADCODE START ---
        operation __InlineApplyIfRelation_70f60d47(q : Qubit[]) : Unit is Adj + Ctl {
            operation __InlineApplyIfEqualAction_5d1ef167(q : Qubit[]) : Unit is Adj + Ctl {
                operation __InlineApplyIfEqualAction_4b498aff(q : Qubit[]) : Unit is Adj + Ctl {
                        Z(q[0]);
                        Rz(4.302059, q[0]);
                        Rx(1.398804, q[0]);
                        ApplyToEachCA(MySingleBlock_78cf336b, q);
                        Z(q[0]);
                        Rz(4.247334, q[0]);
                        S(q[0]);
                }
                ...
                let y = [q[2], q[3]];
                let target = [q[1]];
                ApplyIfEqualLE(__InlineApplyIfEqualAction_4b498aff, x, y, target);
            }
            let x = [q[0]];
            let y = [q[7]];
            let target = [q[1], q[2], q[3], q[5], q[9]];
            ApplyIfEqualLE(__InlineApplyIfEqualAction_5d1ef167, x, y, target);
        }
        use x = Qubit[2];
        X(x[0]);
        use y = Qubit[2];
        X(y[1]);
        let target = q;
        ApplyIfGreaterLE(__InlineApplyIfRelation_70f60d47, x, y, target);
        X(x[0]);
        X(y[1]);
        // --- DEADCODE END ---
    }
