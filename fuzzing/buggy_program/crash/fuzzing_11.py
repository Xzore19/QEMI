...
qr_ecbbfd = QuantumRegister(2)
cr_ecbbfd = ClassicalRegister(2)
qc.add_register(qr_ecbbfd)
qc.add_register(cr_ecbbfd)
qc.x(qr_ecbbfd[0])
qc.x(qr_ecbbfd[1])
qc.measure(qr_ecbbfd[0], cr_ecbbfd[0]) 
qc.measure(qr_ecbbfd[1], cr_ecbbfd[1]) 
with qc.while_loop((cr_ecbbfd, 0b10)): 
	a = 0
	with qc.for_loop(range(a)) as i_f98c5f:
		qc.append(DraperQFTAdder(3), [qreg[3], qreg[5], qreg[0], qreg[4], qreg[2], qreg[1]])
		def fun_e1fcaf(): 
			qc =  QuantumCircuit(3) 
			qc.iswap(2, 1)
			qc.tdg(0)
			qc.cswap(2, 1, 0)
			return qc.to_gate() 
		g_991bd9 = fun_e1fcaf().control(1) 
		qc.append(g_991bd9, [4,1,3,2]) 
		qc.x(4)
		qc.cz(0, 4)
		qc.append(CUGate(0.698, 4.475, 4.525, 2.742), [qreg[0], qreg[2]])
		qc.append(CRXGate(4.661), [qreg[5], qreg[4]])
		qc.cp(1.5707963267948966, 1, 5)
		qc.append(CUGate(2.041, 1.754, 1.919, 0.41), [qreg[5], qreg[4]])
