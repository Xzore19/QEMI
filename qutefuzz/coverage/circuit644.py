from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc0.add_register(qreg_0)
# Adding creg resources 
subcirc0.y(qreg_0[0])
subcirc0.rx(-0.490000, qreg_0[1])
subcirc0.rx(0.655000, qreg_0[2])
subcirc0.y(qreg_0[1])
subcirc0.rx(0.867000, qreg_0[2])

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc1.add_register(qreg_0)
qreg_2 = QuantumRegister(2)
subcirc1.add_register(qreg_2)
# Adding creg resources 
subcirc1.ry(-0.659000, qreg_0[0])
subcirc1.rz(0.234000, qreg_0[1])
subcirc1.y(qreg_0[0])
subcirc1.y(qreg_2[1])
subcirc1.rz(0.241000, qreg_2[1])
subcirc1 = subcirc1.to_gate().control(2)

main_circ = QuantumCircuit(1)
# Adding qregs 
qreg_0 = QuantumRegister(3)
main_circ.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
main_circ.add_register(qreg_3)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")

main_circ.measure(qreg_0[1], creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.ry(0.688000, 0)
		main_circ.y(qreg_0[0])
		main_circ.ry(0.619000, qreg_0[2])
		main_circ.rz(param_0, qreg_0[0])
	with case_1(1):
		main_circ.barrier(qreg_0[1])
main_circ.measure(qreg_0[1], creg_0[0])
with main_circ.if_test((creg_0[0],0)) as else_1:
	main_circ.rz(param_0, qreg_0[2])
	main_circ.append(subcirc0,[qreg_0[0],0,qreg_0[1],qreg_3[0]])
with else_1:
	main_circ.append(subcirc0,[0,qreg_3[0],qreg_0[2],qreg_0[1]])
main_circ.measure(0, creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.rx(param_1, qreg_0[2])
	main_circ.ry(param_0, qreg_3[0])
	main_circ.ry(param_1, qreg_3[0])
	main_circ.ry(0.211000, qreg_0[1])
main_circ.rx(param_0, qreg_0[0])
main_circ.measure(qreg_0[0], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.rx(-0.179000, qreg_0[1])
	main_circ.append(subcirc0,[0,qreg_0[1],qreg_0[0],qreg_3[0]])
main_circ.measure(qreg_3[0], creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.rx(param_0, qreg_0[2])
		main_circ.id(0)
	with case_1(1):
		main_circ.barrier(qreg_3[0])
main_circ.measure(qreg_3[0], creg_0[0])
with main_circ.if_test((creg_0[0],0)) as else_1:
	main_circ.ry(-0.602000, qreg_0[0])
	main_circ.rz(param_1, qreg_0[2])
	main_circ.barrier(qreg_3[0])
with else_1:
	main_circ.y(qreg_0[1])
	main_circ.rz(param_0, 0)
	main_circ.ry(0.893000, qreg_0[0])
main_circ.measure(qreg_0[1], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.append(subcirc0,[qreg_3[0],qreg_0[1],qreg_0[2],qreg_0[0]])
main_circ.y(qreg_0[2])
main_circ.measure(qreg_3[0], creg_0[0])
with main_circ.if_test((creg_0[0],0)):
	main_circ.barrier(qreg_3[0])
main_circ.measure(0, creg_0[1])
with main_circ.if_test((creg_0[1],0)) as else_1:
	main_circ.y(0)
	main_circ.id(qreg_0[1])
with else_1:
	main_circ.ry(param_1, qreg_3[0])
main_circ.y(qreg_3[0])
main_circ.measure(qreg_0[0], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.rx(param_1, qreg_3[0])
	main_circ.id(qreg_0[1])
bindings = {param_0: 0.017000, param_1: 0.738000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "644", "CollectCliffords")
