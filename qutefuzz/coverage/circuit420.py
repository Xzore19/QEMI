from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc0.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
subcirc0.add_register(qreg_1)
# Adding creg resources 
subcirc0.s(qreg_0[0])
subcirc0.y(qreg_0[0])
subcirc0.ry(-0.636000, qreg_1[1])
subcirc0.ry(-0.744000, qreg_1[0])
subcirc0.s(qreg_1[0])
subcirc0.y(qreg_0[0])
subcirc0 = subcirc0.to_gate().control(2)

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc1.add_register(qreg_0)
qreg_2 = QuantumRegister(1)
subcirc1.add_register(qreg_2)
qreg_3 = QuantumRegister(1)
subcirc1.add_register(qreg_3)
# Adding creg resources 
subcirc1.ry(-0.009000, qreg_3[0])
subcirc1.ry(-0.943000, qreg_0[1])
subcirc1.y(qreg_0[0])
subcirc1.y(qreg_3[0])
subcirc1.s(qreg_0[1])
subcirc1.rx(-0.672000, qreg_3[0])

subcirc2 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
subcirc2.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
subcirc2.add_register(qreg_3)
# Adding creg resources 
subcirc2.ry(-0.677000, qreg_0[0])
subcirc2.s(qreg_0[2])
subcirc2.ry(-0.118000, qreg_0[1])
subcirc2.s(qreg_0[1])
subcirc2.ry(-0.051000, qreg_0[0])
subcirc2.ry(-0.122000, qreg_0[2])

subcirc3 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc3.add_register(qreg_0)
qreg_2 = QuantumRegister(2)
subcirc3.add_register(qreg_2)
# Adding creg resources 
subcirc3.y(qreg_0[1])
subcirc3.ry(0.306000, qreg_0[1])
subcirc3.rx(0.955000, qreg_0[1])
subcirc3.ry(-0.928000, qreg_0[1])
subcirc3.y(qreg_0[0])
subcirc3.s(qreg_0[1])

main_circ = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
main_circ.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
main_circ.add_register(qreg_3)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")

main_circ.append(subcirc1,[qreg_0[1],qreg_3[0],qreg_0[0],qreg_0[2]])
main_circ.ry(param_2, qreg_0[0])
main_circ.append(subcirc1,[qreg_3[0],qreg_0[2],qreg_0[1],qreg_0[0]])
main_circ.append(subcirc2,[qreg_0[2],qreg_0[0],qreg_3[0],qreg_0[1]])
main_circ.append(subcirc3,[qreg_0[1],qreg_0[0],qreg_0[2],qreg_3[0]])
main_circ.s(qreg_0[1])
main_circ.s(qreg_0[1])
main_circ.ry(-0.774000, qreg_0[0])
main_circ.rx(param_2, qreg_0[1])
main_circ.y(qreg_0[0])
main_circ.s(qreg_3[0])
main_circ.y(qreg_0[0])
main_circ.append(subcirc1,[qreg_3[0],qreg_0[0],qreg_0[1],qreg_0[2]])
main_circ.rx(param_3, qreg_0[0])
main_circ.append(subcirc2,[qreg_0[2],qreg_0[1],qreg_0[0],qreg_3[0]])
main_circ.s(qreg_0[2])
main_circ.rx(param_2, qreg_0[0])
main_circ.append(subcirc2,[qreg_0[2],qreg_0[1],qreg_3[0],qreg_0[0]])
main_circ.y(qreg_0[2])
main_circ.rx(param_3, qreg_0[2])
main_circ.ry(param_2, qreg_0[0])
main_circ.rx(0.902000, qreg_0[0])
main_circ.rx(0.873000, qreg_3[0])
bindings = {param_2: 0.718000, param_3: -0.941000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
compare_statevectors(main_circ, "RemoveResetInZeroState")
