from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
qreg_1 = QuantumRegister(1)
main_circ.add_register(qreg_1)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")

main_circ.measure(2, creg_1[0])
with main_circ.if_test((creg_1[0],0)):
	main_circ.measure(qreg_0[0], creg_0[0])
	with main_circ.if_test((creg_0[0],0)):
		main_circ.ry(param_1, 1)
		main_circ.u(0,0,param_0, 3)
		main_circ.ry(0.712000, 2)
main_circ.measure(1, creg_0[0])
with main_circ.switch(creg_0[0]) as case_2:
	with case_2(0):
		main_circ.measure(1, creg_1[0])
		with main_circ.switch(creg_1[0]) as case_1:
			with case_1(0):
				main_circ.cy(3,qreg_0[0])
				main_circ.ry(-0.340000, 3)
				main_circ.u(param_0,0,0.475000, 0)
				main_circ.cy(2,3)
			with case_1(1):
				main_circ.rz(param_1, 3)
				main_circ.cy(qreg_0[0],3)
				main_circ.ry(0.044000, 1)
				main_circ.cy(qreg_1[0],qreg_0[0])
	with case_2(1):
		main_circ.measure(3, creg_0[0])
		with main_circ.if_test((creg_0[0],0)) as else_1:
			main_circ.u(0,0,0.722000, qreg_0[0])
			main_circ.cy(3,1)
		with else_1:
			main_circ.u(param_1,param_0,param_0, 3)
			main_circ.ry(param_1, 1)
			main_circ.rz(-0.194000, 0)
			main_circ.ry(param_1, 3)
			main_circ.ry(-0.674000, 3)
main_circ.rz(param_0, 1)
main_circ.measure(0, creg_1[0])
with main_circ.switch(creg_1[0]) as case_2:
	with case_2(0):
		main_circ.ry(-0.903000, 2)
		main_circ.measure(qreg_0[0], creg_0[0])
		with main_circ.switch(creg_0[0]) as case_1:
			with case_1(0):
				main_circ.cy(qreg_1[0],2)
				main_circ.rz(-0.627000, 3)
				main_circ.rz(param_0, 2)
				main_circ.rz(param_0, qreg_1[0])
			with case_1(1):
				main_circ.cy(3,1)
				main_circ.cy(qreg_0[0],3)
				main_circ.ry(param_0, 3)
				main_circ.ry(-0.445000, 0)
	with case_2(1):
		main_circ.measure(0, creg_0[0])
		with main_circ.if_test((creg_0[0],0)):
			main_circ.cy(2,qreg_1[0])
			main_circ.cy(1,0)
		main_circ.cy(3,qreg_1[0])
		main_circ.measure(3, creg_1[0])
		with main_circ.if_test((creg_1[0],0)):
			main_circ.cy(3,qreg_0[0])
			main_circ.cy(3,qreg_1[0])
main_circ.measure(qreg_0[0], creg_1[0])
with main_circ.switch(creg_1[0]) as case_2:
	with case_2(0):
		main_circ.measure(2, creg_1[0])
		with main_circ.if_test((creg_1[0],0)):
			main_circ.u(0,param_0,param_1, 3)
			main_circ.cy(0,qreg_0[0])
			main_circ.cy(1,qreg_0[0])
		main_circ.measure(2, creg_1[0])
		with main_circ.switch(creg_1[0]) as case_1:
			with case_1(0):
				main_circ.ry(param_1, 0)
				main_circ.ry(0.455000, 1)
				main_circ.u(param_0,0,-0.480000, 1)
				main_circ.u(param_1,param_1,-0.048000, 1)
			with case_1(1):
				main_circ.rz(param_0, qreg_0[0])
				main_circ.barrier(1)
	with case_2(1):
		main_circ.id(2)
bindings = {param_0: 0.219000, param_1: 0.719000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "401", "InverseCancellation")
