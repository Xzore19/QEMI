...
subcirc0.x(qreg_3[0])
subcirc0.u(0,0,-0.117000, qreg_0[1])
...
# Adding creg resources 
subcirc1.rz(0.501000, qreg_0[3])
subcirc1.rz(0.572000, qreg_0[3])
subcirc1.rz(-0.729000, qreg_0[0])
subcirc1.cz(qreg_0[1],qreg_0[2])
subcirc1.x(qreg_0[0])
subcirc1.rz(0.090000, qreg_0[1])
...
main_circ.u(0,0,-0.835000, qreg_0[1])
main_circ.measure(qreg_0[0], creg_1[0])
with main_circ.switch(creg_1[0]) as case_1:
	with case_1(0):
		main_circ.barrier(qreg_0[2])
	with case_1(1):
		main_circ.rz(-0.663000, qreg_0[2])
...
with main_circ.switch(creg_0[0]) as case_1:
	...
	with case_1(1):
		...
		main_circ.u(0,param_0,-0.611000, qreg_0[0])
...
with else_1:
	...
	main_circ.cz(qreg_0[2],qreg_0[1])
	main_circ.rz(0.519000, qreg_0[3])
...
with main_circ.if_test((creg_1[0],0)):
	main_circ.rz(0.554000, qreg_0[2])
	main_circ.id(qreg_0[3])
main_circ.measure(qreg_0[1], creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
...
with main_circ.if_test((creg_0[0],0)) as else_1:
	main_circ.rz(-0.714000, qreg_0[1])
with else_1:
	main_circ.barrier(qreg_0[0])
main_circ.x(qreg_0[0])
main_circ.measure(qreg_0[0], creg_1[0])
...
with main_circ.switch(creg_1[0]) as case_1:
	with case_1(0):
		...
		main_circ.x(qreg_0[1])
		main_circ.cz(qreg_0[1],qreg_0[3])
		main_circ.u(param_1,param_2,-0.933000, qreg_0[2])
	with case_1(1):
		main_circ.x(qreg_0[2])
...
bindings = {param_0: -0.250000, param_1: -0.072000, param_2: 0.091000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "1355", "RemoveDiagonalGatesBeforeMeasure")
