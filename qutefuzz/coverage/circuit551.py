from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc0.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
subcirc0.add_register(qreg_1)
# Adding creg resources 
subcirc0.rz(0.479000, qreg_1[2])
subcirc0.h(qreg_1[1])
subcirc0.z(qreg_1[1])
subcirc0.z(qreg_1[1])
subcirc0.rz(1.000000, qreg_1[2])
subcirc0.z(qreg_0[0])
subcirc0 = subcirc0.to_gate().control(1)

main_circ = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
main_circ.add_register(qreg_1)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")

main_circ.h(qreg_1[2])
main_circ.z(qreg_1[2])
main_circ.rz(param_3, qreg_1[1])
main_circ.h(qreg_1[0])
main_circ.h(qreg_1[2])
main_circ.rz(-0.705000, qreg_1[0])
main_circ.cz(qreg_1[0],qreg_1[1])
main_circ.z(qreg_0[0])
main_circ.h(qreg_1[2])
main_circ.rz(0.956000, qreg_1[1])
main_circ.rz(param_2, qreg_1[2])
main_circ.z(qreg_1[0])
main_circ.cz(qreg_1[2],qreg_1[0])
main_circ.h(qreg_0[0])
main_circ.h(qreg_1[2])
main_circ.z(qreg_1[0])
main_circ.cz(qreg_1[0],qreg_1[2])
main_circ.h(qreg_0[0])
main_circ.cz(qreg_1[2],qreg_1[1])
main_circ.z(qreg_1[2])
main_circ.rz(param_2, qreg_1[1])
main_circ.h(qreg_1[0])
main_circ.cz(qreg_1[0],qreg_0[0])
main_circ.z(qreg_1[0])
main_circ.z(qreg_1[2])
main_circ.cz(qreg_1[2],qreg_0[0])
main_circ.cz(qreg_1[1],qreg_0[0])
main_circ.h(qreg_1[0])
main_circ.cz(qreg_1[0],qreg_0[0])
main_circ.rz(param_1, qreg_1[0])
main_circ.rz(param_0, qreg_0[0])
main_circ.cz(qreg_1[2],qreg_1[1])
main_circ.cz(qreg_1[2],qreg_1[1])
main_circ.cz(qreg_1[2],qreg_0[0])
main_circ.cz(qreg_0[0],qreg_1[1])
main_circ.z(qreg_1[2])
main_circ.rz(param_3, qreg_0[0])
main_circ.cz(qreg_1[2],qreg_1[0])
main_circ.h(qreg_1[1])
main_circ.cz(qreg_0[0],qreg_1[2])
main_circ.cz(qreg_0[0],qreg_1[0])
main_circ.rz(-0.678000, qreg_1[2])
main_circ.cz(qreg_1[1],qreg_1[0])
bindings = {param_0: -0.856000, param_1: 0.305000, param_2: -0.067000, param_3: 0.672000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_routing_simulation(main_circ, "551")
