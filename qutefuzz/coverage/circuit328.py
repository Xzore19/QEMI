from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc0.add_register(qreg_0)
# Adding creg resources 
subcirc0.h(qreg_0[1])
subcirc0.h(qreg_0[0])
subcirc0.x(qreg_0[1])
subcirc0.x(qreg_0[1])
subcirc0.h(qreg_0[0])

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
subcirc1.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
subcirc1.add_register(qreg_3)
# Adding creg resources 
subcirc1.x(qreg_0[0])
subcirc1.h(qreg_0[2])
subcirc1.h(qreg_0[1])
subcirc1.u(pi/2,0.870000,-0.973000, qreg_0[1])
subcirc1.x(qreg_0[2])

main_circ = QuantumCircuit(2)
# Adding qregs 
qreg_0 = QuantumRegister(3)
main_circ.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
main_circ.add_register(qreg_3)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")

main_circ.h(qreg_0[1])
main_circ.cx(1,qreg_0[2])
main_circ.h(qreg_3[0])
main_circ.x(qreg_0[0])
main_circ.u(param_2,0.298000,-0.839000, qreg_0[1])
main_circ.cx(qreg_0[1],qreg_3[0])
main_circ.x(qreg_0[2])
main_circ.x(qreg_3[0])
main_circ.append(subcirc0,[1,qreg_0[1],0,qreg_0[0]])
main_circ.x(qreg_3[0])
main_circ.cx(qreg_0[2],qreg_3[0])
main_circ.h(0)
main_circ.x(qreg_0[2])
main_circ.append(subcirc1,[qreg_0[1],1,qreg_0[2],qreg_3[0]])
main_circ.x(1)
main_circ.u(pi/2,param_0,param_1, qreg_0[2])
main_circ.x(qreg_3[0])
main_circ.append(subcirc1,[qreg_0[2],qreg_3[0],1,0])
main_circ.cx(0,qreg_0[1])
main_circ.cx(0,qreg_0[2])
main_circ.cx(qreg_0[2],qreg_3[0])
main_circ.cx(qreg_0[0],qreg_3[0])
main_circ.cx(qreg_3[0],qreg_0[2])
main_circ.cx(qreg_0[2],1)
main_circ.cx(qreg_0[2],qreg_3[0])
main_circ.cx(1,qreg_0[1])
main_circ.cx(qreg_3[0],qreg_0[1])
main_circ.u(pi/2,param_2,param_1, qreg_3[0])
bindings = {param_0: 0.521000, param_1: -0.815000, param_2: 0.328000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_routing_simulation(main_circ, "328")
