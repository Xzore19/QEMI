from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc0.add_register(qreg_0)
# Adding creg resources 
subcirc0.x(qreg_0[2])
subcirc0.u(0,0,-0.773000, qreg_0[1])
subcirc0.u(0,0,-0.784000, qreg_0[2])
subcirc0.y(qreg_0[2])
subcirc0.u(0,0,0.843000, qreg_0[1])

main_circ = QuantumCircuit(2)
# Adding qregs 
qreg_0 = QuantumRegister(4)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")

main_circ.y(qreg_0[1])
main_circ.append(subcirc0,[1,qreg_0[1],qreg_0[3],0])
main_circ.u(0,param_2,0.555000, qreg_0[3])
main_circ.x(0)
main_circ.y(qreg_0[1])
main_circ.append(subcirc0,[qreg_0[0],0,qreg_0[2],qreg_0[3]])
main_circ.append(subcirc0,[qreg_0[2],qreg_0[1],1,qreg_0[3]])
main_circ.x(qreg_0[3])
main_circ.append(subcirc0,[qreg_0[3],0,qreg_0[2],1])
main_circ.append(subcirc0,[qreg_0[1],0,qreg_0[3],qreg_0[0]])
main_circ.cz(qreg_0[2],0)
main_circ.cz(qreg_0[2],1)
main_circ.cz(0,qreg_0[0])
main_circ.cz(qreg_0[3],qreg_0[1])
main_circ.cz(qreg_0[2],qreg_0[0])
main_circ.cz(1,qreg_0[0])
main_circ.cz(qreg_0[1],qreg_0[2])
main_circ.cz(qreg_0[1],qreg_0[2])
main_circ.cz(qreg_0[1],0)
main_circ.cz(qreg_0[0],qreg_0[3])
main_circ.cz(qreg_0[0],1)
main_circ.cz(0,1)
main_circ.cz(qreg_0[0],0)
main_circ.cz(qreg_0[2],qreg_0[0])
bindings = {param_2: -0.451000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_routing_simulation(main_circ, "1914")
