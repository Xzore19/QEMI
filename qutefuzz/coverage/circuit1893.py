from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")

main_circ.measure(1, creg_0[1])
with main_circ.if_test((creg_0[1],0)) as else_1:
	main_circ.ry(param_2, 0)
with else_1:
	main_circ.rx(0.221000, qreg_0[0])
	main_circ.u(0,param_2,param_2, 3)
	main_circ.ry(param_3, 0)
main_circ.measure(qreg_0[0], creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.rx(param_0, 1)
		main_circ.ry(param_3, 3)
		main_circ.rx(param_2, qreg_0[0])
		main_circ.rx(param_0, 2)
	with case_1(1):
		main_circ.z(qreg_0[0])
		main_circ.u(param_2,0,param_0, 2)
		main_circ.rx(0.720000, 3)
		main_circ.z(qreg_0[0])
main_circ.measure(3, creg_0[0])
with main_circ.if_test((creg_0[0],0)):
	main_circ.rx(param_0, 0)
	main_circ.rx(0.854000, 1)
main_circ.u(param_3,0,param_1, qreg_0[0])
main_circ.measure(0, creg_0[1])
with main_circ.switch(creg_0[1]) as case_1:
	with case_1(0):
		main_circ.z(3)
		main_circ.z(1)
		main_circ.rx(param_3, qreg_0[0])
		main_circ.rx(param_0, 1)
	with case_1(1):
		main_circ.u(param_0,0,-0.610000, 3)
		main_circ.ry(param_1, 2)
		main_circ.rx(param_0, 3)
		main_circ.rx(0.827000, 1)
main_circ.u(param_2,0,param_0, 3)
main_circ.measure(2, creg_0[1])
with main_circ.if_test((creg_0[1],0)) as else_1:
	main_circ.u(0,0,-0.731000, 1)
	main_circ.u(param_1,param_1,param_0, 3)
	main_circ.u(0,param_2,-0.364000, 3)
	main_circ.rx(param_3, 0)
	main_circ.u(param_0,param_3,-0.044000, qreg_0[0])
with else_1:
	main_circ.z(2)
	main_circ.ry(param_3, qreg_0[0])
	main_circ.ry(-0.371000, qreg_0[0])
	main_circ.u(param_2,param_1,param_2, 3)
	main_circ.rx(param_1, qreg_0[0])
main_circ.ry(param_0, 1)
main_circ.measure(0, creg_0[0])
with main_circ.if_test((creg_0[0],0)):
	main_circ.u(param_1,0,-0.252000, 2)
	main_circ.ry(param_2, 2)
	main_circ.rx(param_1, 2)
main_circ.z(2)
main_circ.rx(0.638000, 2)
main_circ.measure(1, creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.ry(param_3, 3)
		main_circ.u(0,param_1,param_1, qreg_0[0])
		main_circ.ry(param_3, 1)
		main_circ.u(param_2,param_3,-0.369000, 3)
	with case_1(1):
		main_circ.id(2)
bindings = {param_0: 0.222000, param_1: -0.716000, param_2: -0.448000, param_3: 0.502000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_on_simulator(main_circ, "1893")
