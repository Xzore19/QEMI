from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
subcirc0.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
subcirc0.add_register(qreg_3)
# Adding creg resources 
subcirc0.cz(qreg_0[0],qreg_3[0])
subcirc0.s(qreg_0[2])
subcirc0.cz(qreg_0[0],qreg_0[2])
subcirc0.cz(qreg_0[0],qreg_0[1])
subcirc0.cy(qreg_0[1],qreg_0[0])
subcirc0.cy(qreg_0[0],qreg_0[2])
subcirc0 = subcirc0.to_gate().control(3)

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc1.add_register(qreg_0)
# Adding creg resources 
subcirc1.s(qreg_0[3])
subcirc1.u(pi/2,-0.560000,0.053000, qreg_0[2])
subcirc1.cy(qreg_0[2],qreg_0[1])
subcirc1.cz(qreg_0[3],qreg_0[2])
subcirc1.s(qreg_0[0])
subcirc1.s(qreg_0[1])
subcirc1 = subcirc1.to_gate().control(3)

subcirc2 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc2.add_register(qreg_0)
qreg_2 = QuantumRegister(2)
subcirc2.add_register(qreg_2)
# Adding creg resources 
subcirc2.cz(qreg_2[0],qreg_0[1])
subcirc2.cz(qreg_0[0],qreg_2[1])
subcirc2.s(qreg_2[1])
subcirc2.cy(qreg_2[1],qreg_0[0])
subcirc2.cz(qreg_0[0],qreg_2[1])
subcirc2.s(qreg_0[1])

subcirc3 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc3.add_register(qreg_0)
# Adding creg resources 
subcirc3.u(pi/2,0.897000,-0.673000, qreg_0[2])
subcirc3.s(qreg_0[1])
subcirc3.u(pi/2,0.470000,0.717000, qreg_0[3])
subcirc3.s(qreg_0[3])
subcirc3.s(qreg_0[2])
subcirc3.cz(qreg_0[2],qreg_0[1])
subcirc3 = subcirc3.to_gate().control(2)

main_circ = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
main_circ.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
main_circ.add_register(qreg_3)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")

main_circ.append(subcirc2,[qreg_0[1],qreg_0[0],qreg_3[0],qreg_0[2]])
main_circ.u(pi/2,0.688000,-0.306000, qreg_0[1])
main_circ.s(qreg_0[2])
main_circ.append(subcirc2,[qreg_0[0],qreg_3[0],qreg_0[1],qreg_0[2]])
main_circ.s(qreg_0[2])
main_circ.u(pi/2,param_1,param_0, qreg_0[2])
main_circ.u(pi/2,0.263000,0.904000, qreg_0[0])
main_circ.s(qreg_0[2])
main_circ.cy(qreg_0[1],qreg_0[2])
main_circ.u(param_1,0.090000,-0.828000, qreg_0[1])
main_circ.s(qreg_0[1])
main_circ.s(qreg_0[2])
main_circ.append(subcirc2,[qreg_0[0],qreg_0[1],qreg_3[0],qreg_0[2]])
main_circ.s(qreg_0[2])
main_circ.cy(qreg_3[0],qreg_0[2])
main_circ.append(subcirc2,[qreg_3[0],qreg_0[2],qreg_0[1],qreg_0[0]])
main_circ.s(qreg_3[0])
main_circ.cz(qreg_0[2],qreg_3[0])
main_circ.cz(qreg_3[0],qreg_0[0])
main_circ.cy(qreg_0[0],qreg_3[0])
main_circ.cz(qreg_0[0],qreg_0[2])
main_circ.append(subcirc2,[qreg_0[2],qreg_3[0],qreg_0[1],qreg_0[0]])
main_circ.cy(qreg_3[0],qreg_0[0])
main_circ.cy(qreg_0[1],qreg_3[0])
bindings = {param_0: -0.437000, param_1: -0.727000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
compare_statevectors(main_circ, "CommutationAnalysis")
