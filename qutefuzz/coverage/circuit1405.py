from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc0.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
subcirc0.add_register(qreg_1)
# Adding creg resources 
subcirc0.rz(-0.537000, qreg_1[2])
subcirc0.rz(-0.601000, qreg_1[2])
subcirc0.cy(qreg_1[1],qreg_0[0])
subcirc0.cy(qreg_1[1],qreg_1[0])
subcirc0.ry(0.118000, qreg_1[1])
subcirc0.ry(0.881000, qreg_1[1])

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc1.add_register(qreg_0)
# Adding creg resources 
subcirc1.ry(-0.311000, qreg_0[2])
subcirc1.ry(-0.473000, qreg_0[0])
subcirc1.cy(qreg_0[3],qreg_0[2])
subcirc1.rz(0.785000, qreg_0[1])
subcirc1.cy(qreg_0[2],qreg_0[3])
subcirc1.rz(-0.367000, qreg_0[2])
subcirc1 = subcirc1.to_gate().control(2)

main_circ = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
main_circ.add_register(qreg_1)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")
param_4 = Parameter("param_4")

main_circ.measure(qreg_0[0], creg_0[1])
with main_circ.switch(creg_0[1]) as case_1:
	with case_1(0):
		main_circ.append(subcirc0,[qreg_0[0],qreg_1[2],qreg_1[0],qreg_1[1]])
	with case_1(1):
		main_circ.barrier(qreg_1[2])
main_circ.rz(param_2, qreg_1[2])
main_circ.measure(qreg_1[2], creg_0[1])
with main_circ.if_test((creg_0[1],0)) as else_1:
	main_circ.append(subcirc0,[qreg_1[1],qreg_1[0],qreg_0[0],qreg_1[2]])
with else_1:
	main_circ.cy(qreg_1[2],qreg_1[1])
	main_circ.cy(qreg_1[2],qreg_0[0])
main_circ.measure(qreg_1[1], creg_0[1])
with main_circ.switch(creg_0[1]) as case_1:
	with case_1(0):
		main_circ.id(qreg_1[2])
	with case_1(1):
		main_circ.append(subcirc0,[qreg_1[1],qreg_0[0],qreg_1[2],qreg_1[0]])
main_circ.measure(qreg_0[0], creg_0[1])
with main_circ.switch(creg_0[1]) as case_1:
	with case_1(0):
		main_circ.rz(param_2, qreg_1[2])
		main_circ.barrier(qreg_1[2])
	with case_1(1):
		main_circ.id(qreg_0[0])
main_circ.measure(qreg_1[0], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.rz(0.266000, qreg_1[1])
	main_circ.append(subcirc0,[qreg_0[0],qreg_1[1],qreg_1[2],qreg_1[0]])
main_circ.measure(qreg_1[0], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.cy(qreg_1[0],qreg_0[0])
	main_circ.append(subcirc0,[qreg_1[0],qreg_1[1],qreg_0[0],qreg_1[2]])
main_circ.measure(qreg_1[2], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.cy(qreg_1[1],qreg_1[0])
	main_circ.id(qreg_1[0])
main_circ.measure(qreg_0[0], creg_0[1])
with main_circ.switch(creg_0[1]) as case_1:
	with case_1(0):
		main_circ.u(param_4,-0.705000,param_3, qreg_1[1])
		main_circ.ry(param_3, qreg_1[2])
		main_circ.u(param_1,param_2,param_3, qreg_1[2])
		main_circ.append(subcirc0,[qreg_1[2],qreg_1[0],qreg_1[1],qreg_0[0]])
	with case_1(1):
		main_circ.u(param_2,param_0,param_1, qreg_0[0])
		main_circ.u(param_1,param_0,param_0, qreg_1[1])
		main_circ.barrier(qreg_1[2])
bindings = {param_0: -0.319000, param_1: 0.908000, param_2: 0.136000, param_3: 0.026000, param_4: 0.694000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_on_simulator(main_circ, "1405")
