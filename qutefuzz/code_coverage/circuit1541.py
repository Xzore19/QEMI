from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc0.add_register(qreg_0)
qreg_2 = QuantumRegister(2)
subcirc0.add_register(qreg_2)
# Adding creg resources 
subcirc0.cx(qreg_0[1],qreg_0[0])
subcirc0.cx(qreg_0[1],qreg_2[0])
subcirc0.cz(qreg_2[1],qreg_0[0])
subcirc0.cx(qreg_0[0],qreg_0[1])

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc1.add_register(qreg_0)
qreg_1 = QuantumRegister(1)
subcirc1.add_register(qreg_1)
qreg_2 = QuantumRegister(1)
subcirc1.add_register(qreg_2)
qreg_3 = QuantumRegister(1)
subcirc1.add_register(qreg_3)
# Adding creg resources 
subcirc1.rz(0.666000, qreg_3[0])
subcirc1.x(qreg_3[0])
subcirc1.cx(qreg_3[0],qreg_2[0])
subcirc1.rz(0.502000, qreg_2[0])
subcirc1 = subcirc1.to_gate().control(3)

subcirc2 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc2.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
subcirc2.add_register(qreg_1)
# Adding creg resources 
subcirc2.cx(qreg_1[1],qreg_0[0])
subcirc2.rz(0.718000, qreg_1[0])
subcirc2.rz(0.871000, qreg_1[0])
subcirc2.rz(-0.960000, qreg_0[0])

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")

main_circ.x(0)
main_circ.cx(0,1)
main_circ.append(subcirc0,[3,qreg_0[0],2,1])
main_circ.cz(0,1)
main_circ.cx(1,0)
main_circ.rz(0.332000, qreg_0[0])
main_circ.cx(3,0)
main_circ.rz(param_1, 3)
main_circ.x(2)
main_circ.append(subcirc0,[1,0,qreg_0[0],3])
main_circ.cx(3,2)
main_circ.rz(0.513000, 0)
main_circ.append(subcirc2,[3,2,1,qreg_0[0]])
main_circ.x(1)
main_circ.cz(2,0)
main_circ.cx(0,qreg_0[0])
main_circ.append(subcirc0,[qreg_0[0],0,2,1])
main_circ.cx(qreg_0[0],0)
main_circ.append(subcirc2,[1,0,2,3])
main_circ.append(subcirc0,[1,0,2,3])
main_circ.cx(3,1)
main_circ.cx(qreg_0[0],3)
main_circ.cx(1,qreg_0[0])
main_circ.x(0)
main_circ.cx(2,0)
main_circ.x(2)
main_circ.append(subcirc0,[qreg_0[0],1,0,3])
main_circ.x(1)
main_circ.cx(3,qreg_0[0])
main_circ.cz(0,1)
main_circ.x(3)
main_circ.cz(0,1)
bindings = {param_1: 0.453000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_routing_simulation(main_circ, "1541")
