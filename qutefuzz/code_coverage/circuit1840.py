from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
subcirc0.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
subcirc0.add_register(qreg_3)
# Adding creg resources 
subcirc0.s(qreg_3[0])
subcirc0.cy(qreg_0[0],qreg_0[2])
subcirc0.s(qreg_0[1])
subcirc0.ry(0.572000, qreg_0[1])
subcirc0.s(qreg_0[1])
subcirc0.u(pi/2,0.236000,0.177000, qreg_0[0])

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc1.add_register(qreg_0)
qreg_2 = QuantumRegister(1)
subcirc1.add_register(qreg_2)
qreg_3 = QuantumRegister(1)
subcirc1.add_register(qreg_3)
# Adding creg resources 
subcirc1.ry(0.686000, qreg_0[1])
subcirc1.ry(0.622000, qreg_3[0])
subcirc1.cy(qreg_3[0],qreg_2[0])
subcirc1.cy(qreg_3[0],qreg_0[0])
subcirc1.cy(qreg_0[1],qreg_0[0])
subcirc1.u(pi/2,0.778000,0.501000, qreg_0[1])

subcirc2 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc2.add_register(qreg_0)
# Adding creg resources 
subcirc2.ry(0.380000, qreg_0[2])
subcirc2.u(pi/2,-0.186000,0.933000, qreg_0[3])
subcirc2.cy(qreg_0[3],qreg_0[0])
subcirc2.s(qreg_0[1])
subcirc2.s(qreg_0[0])
subcirc2.ry(0.158000, qreg_0[1])

subcirc3 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc3.add_register(qreg_0)
qreg_1 = QuantumRegister(3)
subcirc3.add_register(qreg_1)
# Adding creg resources 
subcirc3.cy(qreg_1[2],qreg_1[1])
subcirc3.s(qreg_1[1])
subcirc3.s(qreg_1[0])
subcirc3.cy(qreg_1[0],qreg_1[1])
subcirc3.u(pi/2,-0.461000,0.087000, qreg_0[0])
subcirc3.s(qreg_1[2])
subcirc3 = subcirc3.to_gate().control(3)

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")

main_circ.measure(0, creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.id(1)
	with case_1(1):
		main_circ.append(subcirc1,[3,0,1,2])
main_circ.measure(3, creg_1[0])
with main_circ.if_test((creg_1[0],0)):
	main_circ.ry(param_1, 2)
main_circ.cy(3,qreg_0[0])
main_circ.u(param_1,-0.213000,-0.490000, 0)
main_circ.measure(2, creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.u(param_2,param_3,param_0, qreg_0[0])
		main_circ.u(param_3,param_0,param_2, 0)
		main_circ.ry(0.193000, 1)
		main_circ.ry(param_1, 1)
	with case_1(1):
		main_circ.append(subcirc0,[2,qreg_0[0],3,0])
main_circ.measure(2, creg_1[0])
with main_circ.if_test((creg_1[0],0)) as else_1:
	main_circ.append(subcirc0,[qreg_0[0],2,0,1])
with else_1:
	main_circ.append(subcirc2,[3,2,0,qreg_0[0]])
main_circ.measure(1, creg_1[0])
with main_circ.if_test((creg_1[0],0)):
	main_circ.cy(3,2)
main_circ.cy(2,qreg_0[0])
main_circ.measure(0, creg_1[0])
with main_circ.switch(creg_1[0]) as case_1:
	with case_1(0):
		main_circ.cy(0,1)
		main_circ.cy(0,qreg_0[0])
		main_circ.cy(0,3)
		main_circ.barrier(1)
	with case_1(1):
		main_circ.u(pi/2,-0.318000,0.593000, 1)
		main_circ.barrier(2)
main_circ.measure(3, creg_0[0])
with main_circ.switch(creg_0[0]) as case_1:
	with case_1(0):
		main_circ.s(0)
		main_circ.cy(3,qreg_0[0])
		main_circ.cy(2,qreg_0[0])
		main_circ.barrier(qreg_0[0])
	with case_1(1):
		main_circ.barrier(3)
bindings = {param_0: 0.212000, param_1: -0.340000, param_2: 0.509000, param_3: -0.266000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "1840", "RemoveFinalReset")
