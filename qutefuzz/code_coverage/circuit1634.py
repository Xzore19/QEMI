from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")
param_4 = Parameter("param_4")

main_circ.cy(0,3)
main_circ.cy(1,0)
main_circ.y(2)
main_circ.h(0)
main_circ.u(0,0,param_3, 1)
main_circ.cy(1,qreg_0[0])
main_circ.u(param_4,0,-0.825000, 2)
main_circ.cy(0,qreg_0[0])
main_circ.cy(qreg_0[0],1)
main_circ.h(3)
main_circ.cy(0,3)
main_circ.y(qreg_0[0])
main_circ.cy(3,2)
main_circ.y(2)
main_circ.cy(2,1)
main_circ.u(param_1,0,param_0, 0)
main_circ.h(qreg_0[0])
main_circ.cy(qreg_0[0],2)
main_circ.y(qreg_0[0])
main_circ.cy(0,qreg_0[0])
main_circ.cy(3,0)
main_circ.cy(2,0)
main_circ.y(0)
main_circ.y(1)
main_circ.y(2)
main_circ.u(0,param_3,-0.557000, 0)
main_circ.y(1)
main_circ.u(param_1,0,param_0, qreg_0[0])
main_circ.u(0,0,-0.549000, 2)
main_circ.cy(1,2)
main_circ.y(3)
main_circ.h(1)
main_circ.h(2)
main_circ.h(2)
main_circ.u(0,0,param_1, 0)
main_circ.y(2)
main_circ.cy(3,1)
main_circ.cy(3,2)
main_circ.cy(1,2)
main_circ.h(qreg_0[0])
main_circ.h(1)
main_circ.cy(3,0)
main_circ.y(1)
main_circ.u(0,param_2,0.740000, 2)
main_circ.h(2)
main_circ.y(2)
main_circ.h(3)
main_circ.h(2)
main_circ.cy(qreg_0[0],3)
main_circ.h(qreg_0[0])
main_circ.u(param_2,0,param_0, 0)
main_circ.u(0,0,param_1, 2)
bindings = {param_0: 0.858000, param_1: -0.117000, param_2: 0.879000, param_3: 0.443000, param_4: 0.223000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_routing_simulation(main_circ, "1634")
