from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc0.add_register(qreg_0)
# Adding creg resources 
subcirc0.cx(qreg_0[0],qreg_0[1])
subcirc0.cx(qreg_0[3],qreg_0[1])
subcirc0.cz(qreg_0[2],qreg_0[1])
subcirc0.u(0,0,-0.150000, qreg_0[0])
subcirc0 = subcirc0.to_gate().control(1)

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc1.add_register(qreg_0)
# Adding creg resources 
subcirc1.cx(qreg_0[2],qreg_0[0])
subcirc1.u(0.641000,-0.828000,-0.430000, qreg_0[3])
subcirc1.cx(qreg_0[2],qreg_0[3])
subcirc1.u(0,0,-0.234000, qreg_0[0])

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")

main_circ.append(subcirc1,[2,qreg_0[0],1,0])
main_circ.cz(2,qreg_0[0])
main_circ.u(param_2,0.449000,param_1, 1)
main_circ.append(subcirc1,[2,3,1,0])
main_circ.append(subcirc0,[1,3,qreg_0[0],0,2])
main_circ.u(param_0,0.393000,0.382000, 1)
main_circ.cz(1,2)
main_circ.append(subcirc1,[3,2,1,0])
main_circ.cz(2,qreg_0[0])
main_circ.u(param_1,param_2,param_1, qreg_0[0])
main_circ.u(0.771000,0.934000,-0.568000, 0)
main_circ.cx(2,1)
main_circ.cx(1,0)
main_circ.cz(1,3)
main_circ.cx(qreg_0[0],2)
main_circ.append(subcirc1,[1,2,3,qreg_0[0]])
main_circ.u(param_0,param_2,param_1, 0)
main_circ.append(subcirc1,[0,qreg_0[0],3,2])
main_circ.u(0,param_1,-0.312000, 0)
main_circ.cx(3,1)
main_circ.u(param_0,param_1,0.802000, 2)
main_circ.cz(3,0)
main_circ.append(subcirc1,[qreg_0[0],0,2,1])
main_circ.append(subcirc1,[0,2,qreg_0[0],3])
main_circ.cx(0,qreg_0[0])
main_circ.u(0.325000,param_2,param_0, 0)
main_circ.append(subcirc0,[0,qreg_0[0],2,3,1])
main_circ.u(param_2,0.253000,param_1, 2)
bindings = {param_0: 0.414000, param_1: -0.713000, param_2: -0.518000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
compare_statevectors(main_circ, "CollectLinearFunctions")
