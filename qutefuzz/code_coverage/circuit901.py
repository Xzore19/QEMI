from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc0.add_register(qreg_0)
qreg_1 = QuantumRegister(2)
subcirc0.add_register(qreg_1)
qreg_3 = QuantumRegister(1)
subcirc0.add_register(qreg_3)
# Adding creg resources 
subcirc0.u(pi/2,-0.185000,-0.746000, qreg_3[0])
subcirc0.h(qreg_3[0])
subcirc0.cy(qreg_1[1],qreg_0[0])
subcirc0.u(pi/2,0.432000,-0.010000, qreg_1[1])
subcirc0.rz(-0.524000, qreg_3[0])
subcirc0.h(qreg_0[0])

main_circ = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
main_circ.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
main_circ.add_register(qreg_3)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")

main_circ.cy(qreg_0[2],qreg_0[0])
main_circ.h(qreg_3[0])
main_circ.h(qreg_0[0])
main_circ.cy(qreg_0[1],qreg_0[0])
main_circ.u(param_0,0.529000,-0.197000, qreg_3[0])
main_circ.h(qreg_0[2])
main_circ.cy(qreg_3[0],qreg_0[0])
main_circ.cy(qreg_0[0],qreg_0[1])
main_circ.append(subcirc0,[qreg_3[0],qreg_0[2],qreg_0[1],qreg_0[0]])
main_circ.u(param_2,param_0,param_0, qreg_0[0])
main_circ.rz(0.401000, qreg_3[0])
main_circ.u(param_1,-0.435000,-0.362000, qreg_3[0])
main_circ.h(qreg_0[1])
main_circ.append(subcirc0,[qreg_0[1],qreg_0[2],qreg_3[0],qreg_0[0]])
main_circ.u(pi/2,param_2,-0.490000, qreg_0[1])
main_circ.rz(param_0, qreg_3[0])
main_circ.cy(qreg_3[0],qreg_0[2])
main_circ.rz(param_0, qreg_0[1])
main_circ.cy(qreg_3[0],qreg_0[2])
main_circ.rz(param_1, qreg_0[2])
main_circ.cy(qreg_0[1],qreg_3[0])
main_circ.u(param_2,param_2,-0.964000, qreg_0[1])
main_circ.cy(qreg_0[1],qreg_3[0])
main_circ.rz(0.385000, qreg_3[0])
main_circ.append(subcirc0,[qreg_3[0],qreg_0[0],qreg_0[1],qreg_0[2]])
main_circ.cy(qreg_3[0],qreg_0[2])
main_circ.cy(qreg_0[0],qreg_3[0])
main_circ.cy(qreg_3[0],qreg_0[1])
main_circ.cy(qreg_0[1],qreg_3[0])
main_circ.u(pi/2,param_2,param_1, qreg_3[0])
main_circ.cy(qreg_0[0],qreg_0[2])
main_circ.cy(qreg_0[2],qreg_3[0])
main_circ.u(pi/2,0.737000,param_2, qreg_3[0])
main_circ.cy(qreg_0[0],qreg_0[2])
main_circ.cy(qreg_0[0],qreg_0[2])
main_circ.h(qreg_3[0])
bindings = {param_0: -0.372000, param_1: -0.520000, param_2: 0.618000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_routing_simulation(main_circ, "901")
