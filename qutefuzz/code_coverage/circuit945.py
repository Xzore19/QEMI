from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
subcirc0.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
subcirc0.add_register(qreg_3)
# Adding creg resources 
subcirc0.s(qreg_3[0])
subcirc0.s(qreg_0[0])
subcirc0.s(qreg_3[0])
subcirc0.s(qreg_0[0])
subcirc0.s(qreg_0[1])
subcirc0.s(qreg_0[2])

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(1)
main_circ.add_register(creg_0)
creg_1 = ClassicalRegister(1)
main_circ.add_register(creg_1)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")

main_circ.measure(2, creg_0[0])
with main_circ.if_test((creg_0[0],0)):
	main_circ.s(2)
main_circ.measure(1, creg_1[0])
with main_circ.switch(creg_1[0]) as case_1:
	with case_1(0):
		main_circ.ry(-0.261000, 2)
		main_circ.ry(-0.817000, 2)
		main_circ.append(subcirc0,[3,2,0,qreg_0[0]])
	with case_1(1):
		main_circ.ry(param_1, 2)
		main_circ.ry(param_0, 2)
		main_circ.append(subcirc0,[3,2,0,1])
main_circ.measure(2, creg_1[0])
with main_circ.if_test((creg_1[0],0)) as else_1:
	main_circ.append(subcirc0,[qreg_0[0],3,0,1])
with else_1:
	main_circ.append(subcirc0,[1,3,2,0])
main_circ.measure(3, creg_1[0])
with main_circ.if_test((creg_1[0],0)):
	main_circ.append(subcirc0,[3,1,2,qreg_0[0]])
main_circ.ry(0.854000, 3)
main_circ.measure(1, creg_1[0])
with main_circ.switch(creg_1[0]) as case_1:
	with case_1(0):
		main_circ.append(subcirc0,[2,3,1,0])
	with case_1(1):
		main_circ.s(0)
		main_circ.s(2)
		main_circ.s(3)
		main_circ.y(0)
main_circ.measure(0, creg_1[0])
with main_circ.if_test((creg_1[0],0)) as else_1:
	main_circ.u(pi/2,-0.231000,-0.685000, 0)
	main_circ.u(pi/2,param_1,param_1, qreg_0[0])
with else_1:
	main_circ.append(subcirc0,[1,3,0,qreg_0[0]])
main_circ.measure(qreg_0[0], creg_0[0])
with main_circ.if_test((creg_0[0],0)) as else_1:
	main_circ.y(2)
	main_circ.u(param_1,0.417000,param_0, 3)
	main_circ.s(qreg_0[0])
	main_circ.ry(param_1, qreg_0[0])
	main_circ.s(1)
with else_1:
	main_circ.barrier(0)
bindings = {param_0: -0.616000, param_1: 0.652000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "945", "ElidePermutations")
