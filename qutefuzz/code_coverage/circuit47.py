from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc0.add_register(qreg_0)
qreg_1 = QuantumRegister(2)
subcirc0.add_register(qreg_1)
qreg_3 = QuantumRegister(1)
subcirc0.add_register(qreg_3)
# Adding creg resources 
subcirc0.s(qreg_3[0])
subcirc0.s(qreg_3[0])
subcirc0.cy(qreg_0[0],qreg_1[0])
subcirc0.u(0.711000,0.973000,0.341000, qreg_3[0])
subcirc0.cy(qreg_3[0],qreg_1[0])
subcirc0 = subcirc0.to_gate().control(3)

main_circ = QuantumCircuit(2)
# Adding qregs 
qreg_0 = QuantumRegister(4)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")

main_circ.measure(0, creg_0[0])
with main_circ.if_test((creg_0[0],0)):
	main_circ.measure(qreg_0[3], creg_0[1])
	with main_circ.switch(creg_0[1]) as case_1:
		with case_1(0):
			main_circ.cy(1,0)
			main_circ.cy(qreg_0[2],1)
			main_circ.barrier(qreg_0[1])
		with case_1(1):
			main_circ.cz(qreg_0[2],qreg_0[1])
			main_circ.u(param_1,0.015000,param_1, qreg_0[0])
			main_circ.id(1)
main_circ.u(param_0,param_0,param_0, 1)
main_circ.measure(qreg_0[0], creg_0[1])
with main_circ.switch(creg_0[1]) as case_2:
	with case_2(0):
		main_circ.measure(qreg_0[2], creg_0[0])
		with main_circ.if_test((creg_0[0],0)):
			main_circ.u(-0.860000,param_0,param_0, 0)
			main_circ.cy(qreg_0[2],qreg_0[0])
		main_circ.measure(qreg_0[3], creg_0[0])
		with main_circ.switch(creg_0[0]) as case_1:
			with case_1(0):
				main_circ.u(0.289000,-0.884000,param_0, 0)
				main_circ.u(param_0,0.022000,param_0, 1)
				main_circ.cy(qreg_0[2],0)
				main_circ.barrier(qreg_0[3])
			with case_1(1):
				main_circ.id(qreg_0[1])
	with case_2(1):
		main_circ.cy(qreg_0[1],qreg_0[2])
		main_circ.measure(qreg_0[1], creg_0[1])
		with main_circ.if_test((creg_0[1],0)):
			main_circ.id(qreg_0[1])
		main_circ.measure(1, creg_0[1])
		with main_circ.switch(creg_0[1]) as case_1:
			with case_1(0):
				main_circ.barrier(qreg_0[0])
			with case_1(1):
				main_circ.id(qreg_0[3])
		main_circ.measure(qreg_0[2], creg_0[0])
		with main_circ.if_test((creg_0[0],0)):
			main_circ.s(0)
		main_circ.measure(1, creg_0[0])
		with main_circ.if_test((creg_0[0],0)):
			main_circ.cy(0,qreg_0[2])
			main_circ.u(param_1,param_0,param_1, 0)
			main_circ.u(param_1,0.719000,param_0, qreg_0[3])
			main_circ.cz(1,0)
			main_circ.barrier(qreg_0[1])
main_circ.measure(qreg_0[2], creg_0[0])
with main_circ.if_test((creg_0[0],0)) as else_2:
	main_circ.measure(0, creg_0[1])
	with main_circ.switch(creg_0[1]) as case_1:
		with case_1(0):
			main_circ.cy(qreg_0[2],qreg_0[1])
			main_circ.cz(0,qreg_0[0])
			main_circ.id(0)
		with case_1(1):
			main_circ.s(qreg_0[2])
			main_circ.s(qreg_0[0])
			main_circ.cy(qreg_0[2],qreg_0[1])
			main_circ.u(param_1,-0.152000,-0.332000, qreg_0[1])
with else_2:
	main_circ.measure(0, creg_0[0])
	with main_circ.if_test((creg_0[0],0)):
		main_circ.barrier(qreg_0[2])
	main_circ.measure(0, creg_0[0])
	with main_circ.if_test((creg_0[0],0)) as else_1:
		main_circ.cz(qreg_0[1],0)
		main_circ.id(qreg_0[1])
	with else_1:
		main_circ.cz(qreg_0[3],1)
		main_circ.cz(1,0)
main_circ.measure(qreg_0[2], creg_0[0])
with main_circ.switch(creg_0[0]) as case_2:
	with case_2(0):
		main_circ.measure(qreg_0[3], creg_0[1])
		with main_circ.switch(creg_0[1]) as case_1:
			with case_1(0):
				main_circ.cz(qreg_0[0],0)
				main_circ.cy(qreg_0[0],qreg_0[1])
				main_circ.cz(0,qreg_0[1])
				main_circ.s(0)
			with case_1(1):
				main_circ.cy(0,qreg_0[1])
				main_circ.u(param_0,0.958000,param_0, qreg_0[3])
				main_circ.cy(qreg_0[3],0)
				main_circ.id(qreg_0[3])
	with case_2(1):
		main_circ.measure(qreg_0[3], creg_0[1])
		with main_circ.if_test((creg_0[1],0)) as else_1:
			main_circ.cz(qreg_0[2],qreg_0[3])
		with else_1:
			main_circ.cz(1,qreg_0[1])
			main_circ.s(0)
			main_circ.s(qreg_0[2])
			main_circ.id(qreg_0[0])
main_circ.measure(qreg_0[3], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.measure(1, creg_0[0])
	with main_circ.if_test((creg_0[0],0)):
		main_circ.s(qreg_0[1])
		main_circ.u(param_0,-0.009000,-0.762000, 1)
main_circ.measure(qreg_0[3], creg_0[1])
with main_circ.if_test((creg_0[1],0)):
	main_circ.cz(qreg_0[3],1)
	main_circ.measure(qreg_0[3], creg_0[1])
	with main_circ.if_test((creg_0[1],0)) as else_1:
		main_circ.s(0)
		main_circ.cy(qreg_0[0],qreg_0[3])
		main_circ.cy(1,qreg_0[3])
	with else_1:
		main_circ.s(1)
		main_circ.cz(qreg_0[0],0)
		main_circ.id(0)
main_circ.measure(qreg_0[3], creg_0[0])
with main_circ.if_test((creg_0[0],0)):
	main_circ.s(0)
	main_circ.measure(qreg_0[1], creg_0[1])
	with main_circ.switch(creg_0[1]) as case_1:
		with case_1(0):
			main_circ.s(qreg_0[0])
			main_circ.s(qreg_0[2])
			main_circ.s(1)
			main_circ.s(qreg_0[1])
		with case_1(1):
			main_circ.barrier(qreg_0[1])
main_circ.measure(0, creg_0[0])
with main_circ.if_test((creg_0[0],0)) as else_2:
	main_circ.cz(qreg_0[3],0)
	main_circ.measure(1, creg_0[0])
	with main_circ.if_test((creg_0[0],0)):
		main_circ.u(-0.241000,param_1,0.117000, qreg_0[3])
	main_circ.cy(0,1)
	main_circ.measure(qreg_0[2], creg_0[1])
	with main_circ.switch(creg_0[1]) as case_1:
		with case_1(0):
			main_circ.id(1)
		with case_1(1):
			main_circ.cy(qreg_0[2],qreg_0[1])
			main_circ.s(qreg_0[0])
			main_circ.id(qreg_0[2])
with else_2:
	main_circ.cy(qreg_0[0],1)
main_circ.s(0)
main_circ.u(-0.172000,-0.643000,param_0, qreg_0[3])
bindings = {param_0: 0.850000, param_1: -0.697000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_on_simulator(main_circ, "47")
