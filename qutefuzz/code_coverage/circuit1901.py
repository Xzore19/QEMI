from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister
from qiskit.circuit import Parameter, ParameterVector
from helpers.qiskit_helpers import compare_statevectors, run_on_simulator, run_routing_simulation, run_pass_on_simulator
from pathlib import Path
from math import pi

subcirc0 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(4)
subcirc0.add_register(qreg_0)
# Adding creg resources 
subcirc0.x(qreg_0[0])
subcirc0.x(qreg_0[1])
subcirc0.rx(0.136000, qreg_0[1])
subcirc0.y(qreg_0[3])

subcirc1 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(2)
subcirc1.add_register(qreg_0)
qreg_2 = QuantumRegister(2)
subcirc1.add_register(qreg_2)
# Adding creg resources 
subcirc1.rz(0.603000, qreg_2[0])
subcirc1.y(qreg_2[1])
subcirc1.rz(0.791000, qreg_0[1])
subcirc1.rx(0.403000, qreg_0[0])

subcirc2 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(1)
subcirc2.add_register(qreg_0)
qreg_1 = QuantumRegister(2)
subcirc2.add_register(qreg_1)
qreg_3 = QuantumRegister(1)
subcirc2.add_register(qreg_3)
# Adding creg resources 
subcirc2.rx(0.440000, qreg_0[0])
subcirc2.rz(0.993000, qreg_1[1])
subcirc2.y(qreg_0[0])
subcirc2.rz(0.954000, qreg_1[1])

subcirc3 = QuantumCircuit(0)
# Adding qregs 
qreg_0 = QuantumRegister(3)
subcirc3.add_register(qreg_0)
qreg_3 = QuantumRegister(1)
subcirc3.add_register(qreg_3)
# Adding creg resources 
subcirc3.rx(0.694000, qreg_3[0])
subcirc3.x(qreg_0[1])
subcirc3.rz(0.125000, qreg_3[0])
subcirc3.y(qreg_0[2])

main_circ = QuantumCircuit(4)
# Adding qregs 
qreg_0 = QuantumRegister(1)
main_circ.add_register(qreg_0)
# Adding creg resources 
creg_0 = ClassicalRegister(2)
main_circ.add_register(creg_0)
# Adding symbols 
param_0 = Parameter("param_0")
param_1 = Parameter("param_1")
param_2 = Parameter("param_2")
param_3 = Parameter("param_3")
param_4 = Parameter("param_4")

main_circ.append(subcirc1,[0,2,1,3])
main_circ.x(3)
main_circ.append(subcirc1,[2,3,0,qreg_0[0]])
main_circ.x(3)
main_circ.append(subcirc2,[1,0,3,2])
main_circ.rx(0.752000, 2)
main_circ.rz(0.005000, 2)
main_circ.append(subcirc3,[qreg_0[0],2,1,0])
main_circ.rx(param_2, 1)
main_circ.x(3)
main_circ.append(subcirc2,[3,2,0,qreg_0[0]])
main_circ.y(1)
main_circ.x(qreg_0[0])
main_circ.rz(-0.725000, 0)
main_circ.y(qreg_0[0])
main_circ.append(subcirc0,[3,1,0,qreg_0[0]])
main_circ.append(subcirc1,[0,3,2,qreg_0[0]])
main_circ.append(subcirc3,[3,2,0,qreg_0[0]])
main_circ.rx(param_2, 1)
main_circ.append(subcirc1,[2,0,1,qreg_0[0]])
main_circ.y(0)
bindings = {param_2: -0.506000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
compare_statevectors(main_circ, "Optimize1qGates")
