	...
	with main_circ.if_test((creg_0[0],0)) as else_2:
		...
		with main_circ.switch(creg_0[0]) as case_1:
			with case_1(0):
				...
				main_circ.u(param_0,-0.940000,param_3, qreg_2[0])
...
with main_circ.if_test((creg_0[0],0)) as else_3:
	...
	with main_circ.if_test((creg_1[0],0)):
		...
		with main_circ.switch(creg_0[0]) as case_1:
			with case_1(0):
				main_circ.u(param_4,param_1,0.754000, qreg_2[0])
	...
	with main_circ.if_test((creg_0[0],0)) as else_2:
		...
		with main_circ.switch(creg_1[0]) as case_1:
			with case_1(0):
				...
				main_circ.cx(qreg_2[0],qreg_0[1])
			with case_1(1):
				main_circ.u(-0.076000,param_4,param_4, qreg_2[0])
				main_circ.u(0.046000,param_0,param_2, qreg_2[0])
				main_circ.cy(qreg_3[0],qreg_2[0])
				main_circ.z(qreg_3[0])
	with else_2:
		main_circ.measure(qreg_0[1], creg_1[0])
		with main_circ.switch(creg_1[0]) as case_1:
			with case_1(0):
				...
				main_circ.cx(qreg_2[0],qreg_0[1])
				main_circ.u(-0.374000,param_4,param_3, qreg_0[1])
			with case_1(1):
				main_circ.z(qreg_2[0])
				main_circ.z(qreg_0[1])
				main_circ.cx(qreg_0[1],0)
				main_circ.u(0.446000,param_0,param_2, qreg_3[0])
main_circ.measure(qreg_0[1], creg_1[0])
with main_circ.switch(creg_1[0]) as case_3:
		...
		with main_circ.switch(creg_1[0]) as case_2:
			with case_2(0):
				...
				with main_circ.switch(creg_1[0]) as case_1:
					with case_1(0):
						main_circ.cx(qreg_2[0],qreg_0[1])
						main_circ.u(-0.700000,0.112000,-0.246000, qreg_3[0])
						main_circ.z(qreg_0[0])
	...
	with case_3(1):
		...
		with else_2:
			...
			main_circ.barrier(0)
		main_circ.barrier(qreg_0[0])
bindings = {param_0: 0.549000, param_1: 0.702000, param_2: 0.027000, param_3: -0.409000, param_4: -0.244000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_on_simulator(main_circ, "685")
