			...
			with case_2(0):
				...
				with main_circ.switch(creg_0[1]) as case_1:
					...
					with case_1(1):
						...
						main_circ.rz(0.215000, qreg_0[1])
			with case_2(1):
				main_circ.y(qreg_3[0])
...
with else_3:
	...
	with main_circ.if_test((creg_0[1],0)):
		...
		main_circ.measure(qreg_2[0], creg_0[1])
		with main_circ.if_test((creg_0[1],0)) as else_1:
			main_circ.rz(-0.020000, qreg_3[0])
	...
	with case_3(0):
		...
		with main_circ.switch(creg_0[0]) as case_2:
			...
			with case_2(1):
				main_circ.measure(qreg_2[0], creg_0[1])
				with main_circ.if_test((creg_0[1],0)) as else_1:
					main_circ.rz(0.613000, qreg_2[0])
					main_circ.h(qreg_0[1])
					main_circ.y(qreg_3[0])
					main_circ.cz(qreg_0[0],qreg_2[0])
	...
	with case_3(1):
		...
		with else_2:
			...
			with else_1:
				main_circ.barrier(qreg_2[0])
			main_circ.id(qreg_0[1])
bindings = {param_0: 0.145000, param_2: 0.104000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_on_simulator(main_circ, "1952")
