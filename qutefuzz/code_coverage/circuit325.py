...
subcirc0.rx(-0.801000, qreg_0[3])
subcirc0.ry(-0.500000, qreg_0[0])
subcirc0.y(qreg_0[3])
...
# Adding creg resources 
subcirc1.ry(-0.807000, qreg_0[2])
subcirc1.rx(0.813000, qreg_0[3])
subcirc1.cz(qreg_0[3],qreg_0[2])
subcirc1.rx(-0.646000, qreg_0[1])
...
with main_circ.if_test((creg_0[0],0)) as else_4:
	main_circ.rx(-0.367000, qreg_0[2])
	main_circ.measure(qreg_0[3], creg_0[1])
	with main_circ.if_test((creg_0[1],0)):
		main_circ.measure(qreg_0[3], creg_0[1])
		with main_circ.switch(creg_0[1]) as case_2:
			with case_2(0):
				main_circ.rx(-0.035000, qreg_0[0])
		...
		with main_circ.if_test((creg_0[0],0)):
			...
			with main_circ.switch(creg_0[0]) as case_1:
				...
				with case_1(1):
					main_circ.barrier(qreg_0[1])
with else_4:
	main_circ.ry(-0.293000, qreg_0[2])
	main_circ.measure(qreg_0[2], creg_0[0])
	with main_circ.if_test((creg_0[0],0)) as else_3:
		main_circ.measure(qreg_0[1], creg_0[0])
		with main_circ.if_test((creg_0[0],0)):
			...
			with main_circ.switch(creg_0[0]) as case_1:
				with case_1(0):
					...
					main_circ.rx(0.103000, qreg_0[3])
...
bindings = {param_2: 0.927000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "325", "ResetAfterMeasureSimplification")
