...
subcirc1.u(pi/2,-0.071000,-0.317000, qreg_0[2])
subcirc1.y(qreg_0[1])
...
# Adding creg resources 
subcirc2.x(qreg_0[1])
subcirc2.u(pi/2,-0.230000,0.771000, qreg_3[0])
subcirc2.x(qreg_0[1])
...
subcirc4.u(pi/2,-0.455000,-0.392000, qreg_0[3])
		...
		with case_3(1):
			...
			with main_circ.switch(creg_1[0]) as case_2:
				with case_2(0):
					...
					with else_1:
						main_circ.u(pi/2,param_1,-0.011000, qreg_0[0])
		...
		with main_circ.switch(creg_0[0]) as case_2:
			...
			with case_2(1):
				...
				with main_circ.switch(creg_0[0]) as case_1:
					with case_1(0):
						main_circ.y(qreg_0[2])
						main_circ.u(param_2,param_2,-0.554000, qreg_0[1])
						main_circ.y(qreg_0[2])
						main_circ.x(qreg_0[1])
					with case_1(1):
						main_circ.u(param_1,-0.672000,param_1, qreg_0[2])
...
with else_4:
	...
	with main_circ.if_test((creg_0[0],0)):
		...
		with main_circ.if_test((creg_0[0],0)) as else_2:
			...
			main_circ.measure(qreg_0[0], creg_0[0])
			with main_circ.if_test((creg_0[0],0)):
				main_circ.u(pi/2,param_1,-0.684000, qreg_0[1])
				main_circ.x(qreg_0[2])
				main_circ.u(pi/2,0.674000,param_0, qreg_0[0])
				main_circ.id(qreg_0[2])
		with else_2:
			main_circ.measure(qreg_0[0], creg_0[0])
			with main_circ.if_test((creg_0[0],0)):
			...
			with main_circ.switch(creg_1[0]) as case_1:
				...
				with case_1(1):
					main_circ.barrier(qreg_0[0])
main_circ.u(pi/2,param_2,-0.024000, qreg_3[0])
main_circ.measure(qreg_0[1], creg_0[0])
with main_circ.if_test((creg_0[0],0)) as else_4:
	main_circ.measure(qreg_0[2], creg_0[0])
	with main_circ.if_test((creg_0[0],0)):
			...
			with case_2(0):
				...
				with main_circ.switch(creg_1[0]) as case_1:
					with case_1(0):
						main_circ.cx(qreg_0[1],qreg_0[2])
						main_circ.y(qreg_0[2])
						main_circ.u(param_2,-0.914000,param_1, qreg_0[2])
						main_circ.x(qreg_0[2])
					with case_1(1):
						main_circ.y(qreg_0[0])
						main_circ.cx(qreg_3[0],qreg_0[0])
						main_circ.u(pi/2,-0.454000,0.745000, qreg_3[0])
						main_circ.barrier(qreg_0[2])
			with case_2(1):
				main_circ.measure(qreg_0[2], creg_0[0])
				with main_circ.switch(creg_0[0]) as case_1:
					...
					with case_1(1):
						main_circ.x(qreg_3[0])
						main_circ.barrier(qreg_0[1])
				main_circ.u(param_1,param_0,0.076000, qreg_0[2])
				main_circ.barrier(qreg_0[0])
with else_4:
	main_circ.measure(qreg_0[1], creg_0[0])
	with main_circ.if_test((creg_0[0],0)):
		...
		main_circ.barrier(qreg_0[1])
	main_circ.id(qreg_3[0])
bindings = {param_0: -0.990000, param_1: -0.268000, param_2: -0.476000, }
main_circ = main_circ.assign_parameters(bindings)

print(Path(__file__).name, " results:")
main_circ.measure_active()
run_pass_on_simulator(main_circ, "1664", "OptimizeCliffords")
