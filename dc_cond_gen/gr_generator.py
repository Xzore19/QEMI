...
def generate_grover_code(num_qubits=4, cir_name="qc", qreg_name="qreg", creg_name="creg"):
    ...
    body.append(f"grdc_qreg = QuantumRegister({num_qubits})")
    body.append(f"grdc_creg = ClassicalRegister({num_qubits})")
    body.append(f"grdc_qc = QuantumCircuit(grdc_qreg, grdc_creg)")
    body.append(f"grdc_qc.h(grdc_qreg)")
...
if __name__ == "__main__":
    generate_grover_code(num_qubits=4)
