...
cirq.contrib.acquaintance.CircularShiftGate(shift=1, num_qubits=3).on(q[1],q[4],q[3]) 
sub_circuit = cirq.Circuit() 
sub_circuit.append(cirq.H(q[3]).controlled_by(q[4]))
sub_circuit.append(cirq.Z(q[0]))
sub_circuit.append(cirq.SQRT_ISWAP(q[2], q[3]).controlled_by(q[4]))
sub_circuit.append(cirq.S(q[0]))
sub_circuit.append(cirq.MSGate(rads=0.006135923151542565).on(q[0], q[1]).controlled_by(q[3]))
sub_circuit.append(cirq.YYPowGate(exponent=0.01227184630308513).on(q[0], q[3]))
sub_circuit.append(cirq.YPowGate(exponent=0.0030679615757712823).on(q[2]))
sub_circuit.append(cirq.X(q[1]))
sub_circuit.append(cirq.QuantumFourierTransformGate(num_qubits=4).on(q[0],q[2],q[1],q[3]))
sub_circuit.append(cirq.XXPowGate(exponent=0.19634954084936207).on(q[4], q[2]))
...
cirq.contrib.acquaintance.LinearPermutationGate(num_qubits=2, permutation={0: 0, 1: 1}).on(q[0],q[3]) 
...
cirq.contrib.acquaintance.AcquaintanceOperation([q[0],q[1],q[3],q[2],q[4]], [0, 1, 3, 2, 4]) 
cirq.contrib.acquaintance.LinearPermutationGate(num_qubits=5, permutation={4: 1, 1: 0, 0: 3, 3: 2, 2: 4}).on(q[0],q[3],q[1],q[4],q[2]) 
